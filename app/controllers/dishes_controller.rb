class DishesController < ApplicationController
  def index
    ...
    @dishes = @q.result(:distinct => true).includes(:venues, :user, :bookmarked_dishes).page(params[:page]).per(10)
  ...
  def create
    ...
    @dish.time_of_day = params[:time_of_day]
    @dish.cuisine = params[:cuisine]
    @dish.populardishes = params[:populardishes]
    @dish.venues_id = params[:venues_id]
    @dish.user_id = params[:user_id]
    @dish.bookmarked_dishes_id = params[:bookmarked_dishes_id]
